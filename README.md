...
- **真语音通话（跨设备）**：WebRTC P2P 媒体（音频不经服务器）+ STUN 打洞；信令（offer/answer/candidate）经 registry 消息中继，同页双面板与跨设备同一套链路
- **短信（跨设备中继）**：每部电话自己的短信窗 + 消息记录 + 附件；短信经 registry 收件箱投递，跨浏览器/设备可达
...
- 语音：P2P 媒体 + STUN 打洞，信令经 registry 中继（媒体不过服务器；TURN 未部署）
- 消息中继：短信/信令内容经服务方收件箱投递，服务方可见；E2E 加密为演进方向
- 实验性项目 · 社区提案不代表官方

## 文档

- `docs/跨公网语音调试实录.md` — 跨设备语音打通排障实录（4 个真实 bug：信令身份/闭包过期/HTTP 缓存/号码状态，含验证脚本方法）
